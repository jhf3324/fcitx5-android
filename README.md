...
本项目是基于[天鹅输入法](https://github.com/jhf3324/fcitx5-android-swan)的修改版（天鹅输入法本身基于 [Fcitx5 for Android](https://github.com/fcitx5-android/fcitx5-android) 深度定制）。  
...
- 剪贴板同步插件继续补强：
  - `clipboard-sync` 插件新增网络策略限制与界面改进，便于在后台同步、手动推送和多端协作场景下更稳地运行。
...
- 构建与分发链路做了同步整理：
  ...
  - `main` 仍保留自动构建链路，`mod2` 等分支不再因普通提交自动触发对应 CI。
- 最近一轮稳定性修复覆盖了多条高频路径：
  - 修复 vivo 兼容触摸分发路径下的按键动作坐标问题，减少键位点击命中偏移。
  - 修复开机未解锁阶段访问 CE `SharedPreferences` 带来的主题相关问题。
  - 修复长图二维码导入的缩放适配问题。
